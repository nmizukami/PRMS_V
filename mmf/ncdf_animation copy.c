...
//  Define the dimensions.
    retval = nc_def_dim(ncdf, "hru_id_dim", n_hru, &nc_hru_dim);
    if (retval) ERR(retval);

    len = (int)log10(n_hru) + 1; // number of digits needed for string representation of nhru
    retval = nc_def_dim(ncdf, "hru_id_name", len, &nc_hru_name_dim);
    if (retval) ERR(retval);
...
// Set up time variable
    retval = nc_def_var(ncdf, "time", NC_INT, 1, &nc_time_dim, &nc_time_var);
    if (retval) ERR(retval);
    
    ncVarAttributes (ncdf, nc_time_var, "days since 1970-01-01 00:00:00", "", NULL, "time", "time", "");
    
// Build the variable for the HRU name (ie ID as character string)
    hruNameDims[0] = nc_hru_dim;
    hruNameDims[1] = nc_hru_name_dim;
    retval = nc_def_var(ncdf, "HRU_id", NC_CHAR, 2, hruNameDims, &nc_hru_names);
    if (retval) ERR(retval);
    
    ncVarAttributes (ncdf, nc_hru_names, "", "", NULL, "hydrologic reponse unit id", "station_id", "");
    
// Build the variable for the NHM name (ie ID as character string)
    
    if (ani_out_flag > 2) {
        retval = nc_def_dim(ncdf, "nhm_id_name", 6, &nc_nhm_name_dim);
        if (retval) ERR(retval);
        
        hruNameDims[0] = nc_hru_dim;
        hruNameDims[1] = nc_nhm_name_dim;
        retval = nc_def_var(ncdf, "NHM_id", NC_CHAR, 2, hruNameDims, &nc_nhm_names);
        if (retval) ERR(retval);
...
//    printf ("jd_base = %d\n", jd_base);
    ...
    dates = (int *) umalloc(ts_ani * sizeof(int));
    for (i = 0; i < ts_ani; i++) {
        dates[i] = (Mstrttime->jd + i) - jd_base; 
    }
    retval = nc_put_var_int(ncdf, nc_time_var, dates);
...
// Write the HRU IDs as 2D character array.
    fmt[0] = '%';
    fmt[1] = '0';
    sprintf(&fmt[2], "%1d", len);
    fmt[3] = 'd';
    fmt[4] = '\0';
    
    names = (char *) umalloc(n_hru * len * sizeof(char));
    for (i = 0; i < n_hru; i++) {
        sprintf (&(names[i*len]), fmt, i);
    }
    ...
    count[0] = n_hru;
    count[1] = len;
    retval = nc_put_vara_text(ncdf, nc_hru_names, start, count, names);
    if (retval) ERR(retval);
    
// Write the NHM IDs as 2D character array.
    if (ani_out_flag > 2) {
        ...
        count[0] = n_hru;
        count[1] = 6;
        retval = nc_put_vara_text(ncdf, nc_nhm_names, start, count, names);
        if (retval) ERR(retval);
    }
