 ...
 * COMMENT  :
 *
 * $Id: batch_run_functions.c 7741 2015-11-10 20:16:30Z markstro $
 *
-*/
...
typedef struct ncdf_stat_t{
  ...
  int pres_varid, temp_varid;
  char *pres_units, *temp_units;
...
//extern NCDFSTAT *ncdfOpen (void);
//extern int ncdfHeader (NCDFSTAT *);
//extern int ncdfClose (NCDFSTAT *);
//extern int write_nstats (NCDFSTAT *);

extern void ncdfInitAni(long, PUBVAR **);
extern void ncdfUpdateValuesAni (int, DIMEN **, long , PUBVAR **);
extern int ncdfCloseAni(long, PUBVAR **, int);
...
static NCDFSTAT *ncdf; // netcdf statvar struc
...
*/
    if (stats_flag == 1) {
        sprintf(statvar_path, "%s", *((char **) control_var("stat_var_file")));

        if ((statvar_file = fopen(statvar_path, "w")) == NULL) {
            sprintf (err, "ERROR - single_run: Could not open statvar file '%s'\n",
            statvar_path);
            return (err);
        }
...
*/
        fprintf(statvar_file,"%ld\n",nstatVars);

        for (i = 0; i < nstatVars; i++) {
            fprintf(statvar_file,"%s %s\n", statVar_names[i], statVar_element[i]);
        }

    } else if (stats_flag == 2) { //  Write statvars to netcdf file
    // Set up the NCDFSTAT strucutre for netcdf statvar
//        ncdf = ncdfOpen ();
//        ncdfHeader (ncdf);    
    }
...
*/
    if (ani_out_flag > 0) {
        ...
        sprintf(ani_path, "%s", *((char **) control_var("ani_output_file")));
...
*/
    if (ani_out_flag == 1) {
        ...
        for (i = 0; i < naniVars; i++) {
            ...
            if (foobar == ngwdim || foobar == nssrdim) foobar = nhrudim;

            for (j = 0; j < num_ani_dims; j++) {
                if (foobar == ani_out_dims[j]) found = TRUE;
            }
        ...
        for (i = 0; i < num_ani_dims; i++) {
            ...
            if ((ani_out_files[i] = fopen(buf, "w")) == NULL) {
                sprintf (err, "ERROR - single_run: Could not open ani file '%s'\n", buf);
                return (err);
            }
        ...
        // Reset ngw and nssr dimensions to nhru for the purposes of the animation file.
            ...
            for (j = 0; j < num_ani_dims; j++) {
                if (foobar == ani_out_dims[j]) ani_var_files[i] = ani_out_files[j];
            }
        }
    ...
    } else if (ani_out_flag > 1) {
        ncdfInitAni(naniVars, ani_out_vars);
    }
...
*/
                    switch (var->type) {
                        case M_DOUBLE:
                            fprintf (ani_var_files[k], " %10.3e", *((double *) var->value + j));
                        break;

                        case M_FLOAT:
                        /*                           fprintf (ani_var_files[k], "\t%14.6e",
                        */
                            fprintf (ani_var_files[k], " %10.3e", *((float *) var->value + j));
                        break;
...
*/
        ...
        ncdfUpdateValuesAni (num_ani_dims, ani_out_dims, naniVars, ani_out_vars);
    }
...
//        ncdfClose(ncdf);
    ...
    */
    if (ani_out_flag == 1) {
        ...
        ncdfCloseAni(naniVars, ani_out_vars, ani_out_flag);
    }

    closeRuntimeGraphs();

    if (!started) return ("Run period outside of data in file.");


    /*
    * if required, save vars to file
    */
    if (preprocess_on) write_preprocess_params ();
