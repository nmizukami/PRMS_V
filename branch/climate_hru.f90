...
!***********************************************************************
  ...
  IF ( Process(:3)=='run' ) THEN
    ...
    call get_timestep(NCfile_unit, timestep)
    print*,'timestep= ', timestep

    IF ( Climate_temp_flag==1 ) THEN
      call nc_read_var(NCfile_unit, Tmax_unit, Tmaxf, timestep)
      call nc_read_var(NCfile_unit, Tmin_unit, Tminf, timestep)
      Basin_tmax = 0.0D0
      Basin_tmin = 0.0D0
      Basin_temp = 0.0D0
     ENDIF
     ...
     IF ( Climate_precip_flag==1 ) THEN
       call nc_read_var(NCfile_unit, Precip_unit, Hru_ppt, timestep)
        Basin_ppt = 0.0D0
        Basin_rain = 0.0D0
        Basin_snow = 0.0D0
        sum_obs = 0.0D0
      ...
      IF ( Climate_potet_flag==1 ) THEN
        call nc_read_var(NCfile_unit, Et_unit, Potet, timestep)
        Basin_potet = 0.0D0
      ENDIF

      IF ( Climate_swrad_flag==1 ) THEN
        call nc_read_var(NCfile_unit, Swrad_unit, Swrad, timestep)
        Basin_swrad = 0.0D0
      ENDIF

      IF ( Climate_transp_flag==1 ) THEN
        allocate(trans_flag(size(Transp_on)))
        call nc_read_var(NCfile_unit, Transp_unit, trans_flag, timestep)
        Transp_on = int(trans_flag)
        Basin_transp_on = 0
      ENDIF

      IF ( Humidity_cbh_flag==1 ) THEN
        call nc_read_var(NCfile_unit, Humidity_unit, Humidity_hru, timestep)
        Basin_humidity = 0.0D0
      ENDIF

      IF ( Windspeed_cbh_flag==1 ) THEN
        call nc_read_var(NCfile_unit, Windspeed_unit, Windspeed_hru, timestep)
     ...
     &                    Tavgf(i), Tmaxc(i), Tminc(i), Tavgc(i), harea)
          ...
          IF ( Climate_swrad_flag==1 ) THEN
            ...
            Basin_swrad = Basin_swrad + DBLE( Swrad(i)*harea )
          ENDIF
