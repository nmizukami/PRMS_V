...
!   Local Variables
      ...
      DOUBLE PRECISION, SAVE :: Cfs2acft, Ind2mms
...
!   Declared Variables
      ...
      DOUBLE PRECISION, SAVE, ALLOCATABLE :: Hru_outflow_mms(:)
!   Declared Parameters
...
! Declared Variables
     ...
     &     'cfs', Hru_outflow)/=0 ) CALL read_error(3, 'hru_outflow')
      ...
      ALLOCATE ( Hru_outflow_mms(Nhru) )
      IF ( declvar(MODNAME, 'hru_outflow_mms', 'nhru', Nhru, 'double', &
     &     'Total flow leaving each HRU', &
     &     'mm/s', Hru_outflow_mms)/=0 ) CALL read_error(3, 'hru_outflow_mms')
...
!**********************************************************************
      ...
      Ind2mms = 25.4D0/Timestep_seconds
...
! seg variables are not computed if cascades are active as hru_segment is ignored
      ...
      DO jj = 1, Active_hrus
        ...
        Hru_outflow_mms(j) = DBLE( (Sroff(j) + Ssres_flow(j) + Gwres_flow(j)) )*Ind2mms
        IF ( Cascade_flag==0 ) THEN
          i = Hru_segment(j)
          IF ( i>0 ) THEN
...
!***********************************************************************
      IF ( In_out==0 ) THEN
        ...
        WRITE ( Restart_outunit ) Cfs2acft, Ind2mms
        ...
        WRITE ( Restart_outunit ) Hru_outflow, Hru_outflow_mms
      ...
      ELSE
        ...
        READ ( Restart_inunit ) Cfs2acft, Ind2mms
        ...
        READ ( Restart_inunit ) Hru_outflow, Hru_outflow_mms
